...
A second limitation is the size of the dataset relative to the GPU overheads on AiMOS. The local analytics kernel does benefit from GPU execution, but the full application remains dominated by fixed costs such as CUDA runtime initialization and host-device transfers. This means the present report captures an important hybrid-programming result, but not yet a case where the GPU wins end-to-end. Larger datasets, repeated analyses over the same resident device data, or heavier local kernels would likely change that balance. The current implementation therefore demonstrates a correct hybrid workflow more clearly than it demonstrates a production-ready acceleration point.

There are also modeling choices in the preprocessing stage that simplify the problem. The binary trip-record representation is intentionally compact and does not preserve every field that might be useful in later analyses. Likewise, route-type normalization is necessary to compare providers cleanly, but it compresses some of the variety present in extended GTFS route codes. These choices are reasonable for the current assignment because they make the distributed analysis simpler and more measurable, but they also limit the granularity of the final provider comparison.

Finally, the current provider set is broad enough to support comparison but still limited in geographic scope and time horizon. The study covers several major northeastern and midwestern agencies, but it does not yet include a larger national sample, seasonal variation, or historical snapshots over multiple service periods. Expanding the provider set would make the application findings more general, and expanding the time horizon would make the performance study more demanding in exactly the way that could better justify GPU acceleration.
...
extended route-type values, as well as validating that the resulting provider summaries were plausible. In the later stages
