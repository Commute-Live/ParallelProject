...
\[
\langle \texttt{city\_id}, \texttt{feed\_id}, \texttt{route\_type}, \texttt{route\_hash},
\texttt{service\_mask}, \texttt{direction\_id}, \texttt{start\_secs},
\texttt{end\_secs}, \texttt{duration\_secs}, \texttt{stop\_count},
\texttt{trip\_hash}, \texttt{reserved} \rangle
\]
...
\[
(\texttt{city\_id}, \texttt{route\_hash}, \texttt{direction\_id}, \texttt{start\_secs}, \texttt{trip\_hash})
\]
...
The preprocessing stage is implemented in Python. It reads \texttt{routes.txt}, \texttt{trips.txt}, \texttt{stop\_times.txt}, and calendar metadata and then derives one record per trip. During implementation, several real-world GTFS packaging irregularities had to be handled explicitly:
\begin{itemize}[leftmargin=*]
...
The cluster-side timing study completed successfully after the build issues in the hybrid CUDA path were resolved. The figures in this section were generated from the AiMOS output stored in \texttt{experiment\_results.csv}. The observed performance story is more nuanced than the original expectation of a straightforward GPU win: the local CUDA compute kernel is fast, but the end-to-end wall-clock time of the GPU path is dominated by GPU setup and runtime overhead that is not present in the CPU-only MPI baseline. That distinction is important because the assignment asks for total performance analysis, not just kernel timings.
